...
cdef extern from "coco.h":
    ...
    coco_problem_t *naco_get_problem_bbob(const size_t function, const size_t dimension, const size_t instance)
    coco_problem_t *naco_get_problem_bbob_mixint(const size_t function, const size_t dimension, const size_t instance, const size_t period, const int *cardinalities)
    double *naco_problem_get_best_parameter(const coco_problem_t *problem)
    double naco_problem_get_best_value(const coco_problem_t *problem)
...
cdef class Problem:
    ...
    cdef np.ndarray _my_best_parameter
    cdef double _best_value
    ...
    def create_problem_bbob(self, size_t function, size_t dimension, size_t instance):
        self._suite_name = "bbob"
        self.problem = naco_get_problem_bbob(function, dimension, instance)
        return self._initialize(self.problem)
    def create_problem_bbob_mixint(self, size_t function, size_t dimension, size_t instance, cs):
        self._suite_name = "bbob-mixint"
        sz = np.size(cs)
        self.problem = naco_get_problem_bbob_mixint(function, dimension, instance, sz, <int *>np.PyArray_DATA(cs))
        return self._initialize(self.problem)
    cdef _initialize(self, coco_problem_t* problem, free=True):
        ...
        self._best_value = naco_problem_get_best_value(self.problem)
        self._my_best_parameter = np.zeros(self._number_of_variables)
        bp = naco_problem_get_best_parameter(self.problem)
        for i in range(self._number_of_variables):
            self._my_best_parameter[i] = bp[i]
        self.initialized = True
        return self
    ...
    def best_parameter(self):
        return self._my_best_parameter
    @property
